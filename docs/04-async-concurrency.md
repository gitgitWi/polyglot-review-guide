...
> [!NOTE]
> **머신당 동시성의 실전 임팩트** — 고루틴의 "저렴한 동시성"이 규모에서 무엇을 바꾸는지는 최근 대형 서비스들이 Python/Node.js 백엔드를 Go로 옮긴 사례에서 드러납니다. 아래 수치는 한 기술 영상이 정리·보고한 값입니다([출처](https://www.youtube.com/watch?v=-Z813pHqSFI)).
>
> - **Reddit** — 오래된 Python 모놀리스의 코어 도메인(댓글·계정·포스트·서브레딧)을 Go 서비스로 분해. 핵심 write 작업의 P99 레이턴시가 절반으로 줄고, 최대 15초까지 걸리던 댓글 작성 지연을 제거.
> - **Lovable** — 채팅 요청 하나가 모델 프로바이더·내부 서비스·스토리지로 50개 이상의 HTTP 콜을 동시에 날리는 워크로드. Python 약 4.2만 줄을 Go로 재작성한 뒤 서버 인스턴스 200개 → 10개, 배포 시간 15분 → 3분.
> - **Uber** — "같은 트래픽을 더 적은 서버에서" 처리하려 Go로 이행. 같은 작업을 약 97% 적은 컴퓨트로 처리하고, 운영·장애 대응에 쓰던 시간 비중이 60% → 20%로 감소.
>
> 공통 동인은 성능 그 자체가 아니라 **머신당 동시 처리량(concurrency per machine)** 입니다. 앞서 본 요청당 스레드(thread-per-request) 모델이나 단일 스레드 이벤트 루프가 감당하던 부하를, 고루틴은 훨씬 적은 자원으로 소화합니다. 다만 리뷰어 관점에서 이 이점은 앞서 짚은 **고루틴 누수·컨텍스트 취소·채널 데드락**을 제대로 통제했을 때에만 유지된다는 점을 기억하세요.
