...
tags: [code-review, architecture, roadmap, runtime-model]
---
...
## 왜 TypeScript 개발자에게 Go는 유독 가까운가

Kotlin이 JVM 생태계의 관례를 요구하는 것과 달리, Go는 TypeScript 개발자에게 학습 곡선이 완만한 편입니다. 이 점은 마이크로소프트가 **TypeScript 컴파일러 자체를 Go로 포팅**하기로 한 결정(네이티브 포트가 완성되면 TypeScript 7로 릴리스 예정)에서 잘 드러납니다. 빌드와 에디터 로드 타임 개선이 목표였는데, 여러 후보 언어 중 Go를 고른 공식 근거는 이 가이드의 독자에게도 그대로 적용됩니다.

- **코드 패턴이 TypeScript와 닮았다 (핵심 근거)**: 공식 FAQ의 표현대로 "관용적인 Go 코드가 기존 TypeScript 컴파일러의 코딩 패턴과 매우 흡사해 포팅을 훨씬 수월하게" 만들었습니다. 새 언어로의 리라이트가 아니라 구조를 보존한 *포트*가 가능했다는 뜻입니다.
- **GC와 메모리 제어**: Go는 GC를 제공해 "코드베이스 전반이 메모리 관리에 매달리지 않고도" 메모리 레이아웃·할당을 잘 제어할 수 있습니다. 반대로 메모리 관리 모델을 근본부터 다시 설계해야 하는 언어는 포트가 아니라 '밑바닥부터 재작성'이 됩니다. (Hejlsberg는 인터뷰에서 이를 GC가 없는 Rust, 객체지향이 강제되어 함수형 스타일 컴파일러와 맞지 않는 C#의 예로 설명했습니다.)
- **네이티브 컴파일**: Go는 네이티브 코드로 컴파일되어 성능을 확보하며, 그래프·트리 순회가 많은 컴파일러 워크로드에도 잘 맞습니다.

공식 수치는 타입 체크/빌드 최대 약 10배 단축(예: VS Code 코드베이스 77.8초 → 7.5초), 에디터에서 프로젝트 전체 로드 9.6초 → 1.2초(약 8배)입니다. 요점은 "기존 구조를 최대한 유지하면서 성능만 크게 올리는 최단 경로"로 Go가 선택됐다는 것 — 바꿔 말하면 **TypeScript에서 쌓은 감각을 버리지 않고도 읽어낼 수 있는 언어**라는 신호입니다. Go 코드가 처음엔 낯설어도, 이 가이드가 다루는 관점만 익히면 리뷰까지 도달하는 거리가 생각보다 짧습니다.

> [!NOTE]
> 출처: Microsoft DevBlogs [A 10x Faster TypeScript](https://devblogs.microsoft.com/typescript/typescript-native-port/) (Anders Hejlsberg, 2025-03-11) · 공식 FAQ [Why Go?](https://github.com/microsoft/typescript-go/discussions/411) · 네 회사 사례 종합 영상 [The Real Reason Big Tech Is Switching to Go](https://www.youtube.com/watch?v=-Z813pHqSFI)

## 가이드 활용 로드맵

가이드는 세 단계로 구성됩니다. 순서대로 읽어 감각을 쌓아도 좋고, 리뷰 중 필요한 주제만 골라 참조해도 됩니다.

**1단계 · 공통 기초 (Common Fundamentals)**

1. **Syntax Map**: 변수·함수·클래스·가시성 등 언어별 구문을 TypeScript와 1:1로 빠르게 대조합니다.
2. **Types and Modeling**: TypeScript에서 정밀한 스키마를 설계하던 감각을 Kotlin의 data class와 Go의 struct·인터페이스에 적용하는 방법을 확인합니다.
3. **Functions and Control Flow**: `when` 식, 스코프 함수, 컬렉션 연산, Go의 조기 반환과 `defer` 등 흐름 제어의 차이를 익힙니다.
4. **Async and Concurrency**: 싱글 스레드 이벤트 루프와 멀티 스레드/코루틴/고루틴 환경의 실행 흐름 및 동시성 제어 차이를 비교합니다.
5. **Validation and Errors**: 런타임 스키마 검증과 예외/에러 전파 파이프라인을 세 언어 관점에서 정렬합니다.

**2단계 · 언어별 심화 (Language Deep-dives)**

6. **Kotlin Idioms / Go Idioms**: JS/TS에는 대응물이 없어 처음 보면 추측하기 어려운 각 언어 고유 문법(sealed class·object·확장 함수·`by` 위임, 그리고 포인터·구조체 임베딩·채널·`iota` 등)을 읽는 법을 익힙니다. 낯선 기호에서 리뷰가 막히지 않도록 하는 것이 목표입니다.
7. **Kotlin Ecosystem / Kotlin Spring Review Guide**: JVM 인프라와 Spring 아키텍처 규칙, 트랜잭션·JPA·프록시의 위험 요소를 점검합니다.
8. **Go Ecosystem / Go Service Review Guide**: 표준 라이브러리 중심 문화와 명시적 HTTP 서비스의 진입점·미들웨어·핸들러 규칙을 검증합니다.

**3단계 · 통합 점검**

9. **AI Code Review Checklist**: 생성형 AI가 작성한 코드의 논리적 결함이나 미세한 에러 경로 누락을 잡아내기 위한 최종 체크리스트로 활용합니다.

> [!TIP]
> 각 문서에는 주제 태그(예: `null-safety`, `transactions`, `concurrency`)가 달려 있습니다. 상단 검색창에 태그명을 입력하거나 문서 상단의 태그 칩을 눌러 같은 주제의 문서를 모아 볼 수 있습니다.
