...
Kotlin이 JVM 생태계의 관례를 요구하는 것과 달리, Go는 TypeScript 개발자에게 학습 곡선이 완만한 편입니다. 이 점은 마이크로소프트가 **TypeScript 컴파일러 자체를 Go로 포팅**하기로 한 결정(Go 기반 컴파일러를 사용하는 TypeScript 7)에서 잘 드러납니다. 빌드와 에디터 로드 타임 개선이 목표였는데, 여러 후보 언어 중 Go를 고른 근거는 이 가이드의 독자에게도 그대로 적용됩니다.

- **GC가 있다**: 기존 컴파일러는 가비지 컬렉션에 의존하는 구조입니다. Rust처럼 GC가 없는 언어로 옮기면 메모리 관리 모델을 전면 재설계해야 해 사실상 풀 리라이트가 됩니다. Go는 GC를 갖추고 있어 기존 코드 구조를 최대한 보존할 수 있었습니다.
- **함수형 스타일과의 궁합**: 기존 컴파일러가 함수형 스타일로 작성돼 있어, 객체지향이 강제되는 C#으로는 포팅이 아니라 리라이트가 필요했습니다. Go는 이 제약이 덜했습니다.
- **네이티브 컴파일 + TS와 유사한 문법**: Go는 네이티브 코드로 컴파일되어 성능을 확보하면서도, 문법과 스타일이 TypeScript와 꽤 유사해 기존 팀이 빠르게 적응할 수 있었습니다.

보고된 결과는 빌드 약 10배 단축, 에디터 로드 타임 9.5초 → 1.2초입니다. 요점은 "기존 구조를 최대한 유지하면서 성능만 크게 올리는 최단 경로"로 Go가 선택됐다는 것 — 바꿔 말하면 **TypeScript에서 쌓은 감각을 버리지 않고도 읽어낼 수 있는 언어**라는 신호입니다. Go 코드가 처음엔 낯설어도, 이 가이드가 다루는 관점만 익히면 리뷰까지 도달하는 거리가 생각보다 짧습니다. (출처: [The Real Reason Big Tech Is Switching to Go](https://www.youtube.com/watch?v=-Z813pHqSFI))
